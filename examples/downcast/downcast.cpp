...
struct VehicleBinding : public Binding<VehicleBinding, Vehicle> {
  ...
  {
    static luaL_Reg members[] = {{"Start", Start}, {nullptr, nullptr}};
    return members;
  }
...
struct CarBinding : public Binding<CarBinding, Car> {
  ...
  {
    static luaL_Reg members[] = {
        {"Start", Start}, {"LoadGroceries", LoadGroceries}, {nullptr, nullptr}};
    return members;
  }
...
int main()
{
  ...
  run(L,
      "if p == nil then print( 'Motorbike is not a car. Can not load "
      "groceries. Go have fun instead.' ) end");
  run(L, "print(p)");
