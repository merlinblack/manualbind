...
function Button : init(x, y, text) Button.__base
                      .init(self, x, y, 100, 50, 'Button '..text) self.text =
    text end

    -- Override default Widget methods to give default Button actions
    !function Button
    : OnAction() print(
          self.name.." I've been activated!") return true --Event handled end

      function Button : OnHover() print(self.name..' Light up') end

                        function Button
    : OnHoverExit() print(self.name..' Extinguished!') end

      function Button.Create(x, y, text) local btn = {} setmetatable(btn,
                                                                     Button) btn
    : init(x, y, text) return btn end

      -- A dialog box widget to hold buttons
          .--DialogBox = {} DialogBox.__index = DialogBox DialogBox.__base =
        Widget setmetatable(DialogBox, Widget)

function DialogBox.Create(x,
                          y,
                          w,
                          h,
                          title) local dlg = {} setmetatable(dlg, DialogBox) dlg
    : init(x, y, w, h, 'DialogBox '..title) return dlg end

      -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -

      function test()-- A dialog box with two buttons.

      mybox = DialogBox(10, 10, 500, 300, '"Save Game?"') ok =
    Button(20, 150, 'Ok') cancel = Button(150, 150, 'Cancel') mybox
    : AddChild(ok) mybox : AddChild(cancel)

                               print('User moves to outside dialog box') mybox
    : MouseMove(5, 5) print('-------------------------------------')

          print('User moves mouse over "ok"') mybox
    : MouseMove(22, 153) print('-------------------------------------')

          print('User moves to "cancel"') mybox
    : MouseMove(155, 156) print('-------------------------------------')

          print('User clicks left button') mybox
    : MouseClick(Mouse.Button.Left, 156, 154)
          print('-------------------------------------')
